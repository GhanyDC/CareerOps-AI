...
export async function cleanupAuthenticationUser(userId: string) {
  ...
  await authTestClient.jobDuplicateEvent.deleteMany({ where: { userId } });
  await authTestClient.jobDuplicateGroup.deleteMany({ where: { userId } });
  await authTestClient.jobDuplicateCandidate.deleteMany({ where: { userId } });
